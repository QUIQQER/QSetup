...
class Validator
{
    ...
    public static function validatePassword($string)
    {
        ...
        if (empty($string)) {
            throw new SetupException(Installer::getLocale()->getStringLang("validation.password.empty",
                "The given password can not be empty!"));
        }

        if (strlen($string) < $conf['requirements']['pw_min_length']) {

            $msg = Installer::getLocale()->getStringLang("validation.password.minlength",
        ...
        if (self::getUppercaseCount($string) < $conf['requirements']['pw_must_have_uppercase']) {
            $msg = Installer::getLocale()->getStringLang("validation.password.uppercasecount",
        ...
        if (self::getSpecialcharCount($string) < $conf['requirements']['pw_must_have_special']) {
            $msg = Installer::getLocale()->getStringLang("validation.password.specialcount",
        ...
        if (self::getNumberCount($string) < $conf['requirements']['pw_must_have_numbers']) {
            $msg = Installer::getLocale()->getStringLang("validation.password.numbercount",
